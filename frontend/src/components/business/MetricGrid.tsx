      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-5">
        {metrics.map((m, i) => (
          <div key={i} className={`min-w-0 rounded-control border p-4 ${TREND_BG[m.trend] || "border-border bg-surface"}`}>
            ...
            <p className="mt-1 block w-full overflow-hidden whitespace-nowrap text-[clamp(1.0625rem,3.6vw,1.875rem)] font-semibold leading-tight text-ink tabular-nums [text-overflow:clip]" title={m.value}>
              {m.value}
            </p>
