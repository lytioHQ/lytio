...
export default function RiskList({ risks, lang }: { risks: RiskData[]; lang: UILanguage }) {
  ...
  return (
    <div>
      <p className="text-[11px] font-semibold uppercase tracking-wider text-slate-400 mb-3">{T("biz.risksTitle", { n: risks.length })}</p>
          ...
          <div key={i} className={`rounded-xl border ${SEV_COLORS[item.severity] || "border-slate-200"} p-5`}>
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2"><p className="text-sm font-semibold text-slate-800">{item.title}</p><span className="rounded-full bg-white px-2 py-0.5 text-[10px] font-medium text-slate-500 capitalize">{SEV_KEYS[item.severity] ? T(SEV_KEYS[item.severity]) : item.severity}</span></div>
